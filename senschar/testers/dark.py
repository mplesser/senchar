...
from senschar_console.plot import plt
...
class Dark(Tester):
    ...
    def acquire(self):
        ...
        senschar.log("Acquiring dark sequence")

        exposure, tempcon = senschar_console.utils.get_tools(["exposure", "tempcon"])
        ...
        currentfolder, newfolder = senschar_console.utils.make_file_folder("dark")
        ...
        for imgnum in range(self.number_images_acquire):
            # pre-dark bias
            senschar.db.parameters.set_par("imagetype", "dark")  # for GetFilename
            filename = os.path.basename(exposure.get_filename())
            senschar.log(f"Taking pre-dark image: {filename}")
            temp = tempcon.get_temperatures()
            senschar.log(f"Current temperatures: {temp}")
        ...
        senschar.log("Dark sequence finished")
    ...
    def analyze(self):
        ...
        senschar.log("Analyzing dark signal")
        ...
        currentfolder = senschar.util.curdir(subfolder)  # move to analysis folder
        ...
        num_exts, _, _ = senschar.fits.get_extensions(zerofilename)
        self._numchans = max(1, num_exts)
        bin1 = senschar.fits.get_keyword(zerofilename, "CCDBIN1")
        bin2 = senschar.fits.get_keyword(zerofilename, "CCDBIN2")
        ...
        if numdarks == 1:
            ...
            if self.overscan_correct:
                senschar.fits.colbias(masterbias, fit_order=self.fit_order)
        else:
        ...
        if numdarks == 1:
            ...
            if self.overscan_correct:
                senschar.fits.colbias(masterdark, fit_order=self.fit_order)
        else:
            senschar.fits.combine(
                darklist,
                masterdark,
        ...
        senschar.fits.sub(masterdark, masterbias, self.darksub_filename, "float32")

        # create dark senschar image
        self.dark_image = senschar.image.Image(self.darksub_filename)

        # get header info
        exptime = float(senschar.fits.get_keyword(masterdark, "EXPTIME"))
        try:
            self.temperature = float(senschar.fits.get_keyword(masterdark, "CAMTEMP"))
        except Exception:
            self.temperature = -999

        # set scale from gain
        history = senschar.fits.get_history(masterdark)
        if "SCALED" not in history:
            senschar.log("Scale by gain values")
        ...
        if self.grade_dark_signal:
            senschar.log(f"Dark signal grade = {self.dark_signal_grade}")
        else:
        ...
        senschar_console.plot.plt.title("Bright Pixel Rejection Mask")
        self.bright_mask = numpy.ma.getmask(self.masked_image).astype("uint8")
        implot = senschar_console.plot.plt.imshow(self.bright_mask)
        implot.set_cmap("gray")
        senschar_console.plot.plt.show()
        senschar_console.plot.save_figure(fignum, "BrightPixelRejectionMask")

        # write mask as FITS
        maskfile = senschar.image.Image(self.dark_filename)
    ...
    def plot_dark_signal(self):
        ...
        if self._numchans == 1:
            ...
            raise senschar.exceptions.SenscharError("missing subplot configuration")

        # plot dark image
        fig = plt.figure()
        fignum = fig.number
        senschar_console.plot.move_window(fignum)
        senschar_console.plot.plot_image(self.dark_image, "sdev", 10.0)
        plt.title("Combined Dark Image")
        plt.show()
        senschar_console.plot.save_figure(fignum, self.darkimage_plot)
        ...
        plt.show()
        senschar_console.plot.save_figure(fignum, self.cumm_hist_plot)
        ...
        plt.show()
        senschar_console.plot.save_figure(fignum, self.total_hist_plot)
